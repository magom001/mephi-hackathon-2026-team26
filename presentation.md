...
Разные классы моделей предъявляют разные требования к данным.

```
tree_pipe:
   data -> SimpleImputer(median) -> model
   используется в: HGB, CatBoost, LightGBM

kernel_pipe:
   data -> SimpleImputer(median) -> DropConstantColumns
                                 -> StandardScaler
                                 -> model
   используется в: Ridge, Gaussian Process
```

**Почему два пайплайна.** Деревья инвариантны к масштабу и нечувствительны к константам. Ядерные методы зависят от масштаба (для обусловленности матрицы ядра) и не переносят константных признаков (нулевая дисперсия даёт деление на ноль при стандартизации).
...
На SI ошибка 788 при типичных значениях около 5 - линейная модель здесь не работает. Это подтверждает выводы EDA:

- Не умеет ловить взаимодействия признаков (ограничение 3 из карты).
- Чувствительна к выбросам через L2-функцию потерь (ограничение 2).

Дальше нужны модели, которые решают обе проблемы.

---

## Бустинги с L1 и бэггингом

**Ответ на ограничение 3.** Берём градиентный бустинг по деревьям - три семейства с разными деталями реализации:

- HistGradientBoosting (sklearn) - loss = absolute_error
- CatBoost - loss_function = MAE
- LightGBM - objective = regression_l1

**Ответ на ограничение 2.** Все три модели обучаем с функцией потерь L1.

**Ответ на ограничение 1.** Оборачиваем каждый бустинг в bagging:
...
**Сигнал распределён по десяткам признаков, нет одного доминирующего.** Это согласуется с выводом EDA (ограничение 3 из карты): полезная информация в комбинациях. Отбор признаков по одиночной важности ухудшил бы качество - используем все 210.
...
Основная польза - от трёх шагов: деревья вместо линейной модели, L1 вместо L2, добавление GPR к бустингам.
